...
exports.calculateFootprint = calculateFootprint;
...
function calculateFootprint(inputs) {
    ...
    const carEmissions = (Number(inputs.carKm) || 0) * weeksPerMonth * exports.EMISSION_FACTORS.transportation.car;
    const bikeEmissions = (Number(inputs.bikeKm) || 0) * weeksPerMonth * exports.EMISSION_FACTORS.transportation.bike;
    const busEmissions = (Number(inputs.busKm) || 0) * weeksPerMonth * exports.EMISSION_FACTORS.transportation.bus;
    const trainEmissions = (Number(inputs.trainKm) || 0) * weeksPerMonth * exports.EMISSION_FACTORS.transportation.train;
    const transportationTotal = carEmissions + bikeEmissions + busEmissions + trainEmissions;
    // Electricity emissions (monthly kWh * factor)
    const electricityTotal = (Number(inputs.electricityKwh) || 0) * exports.EMISSION_FACTORS.electricity.kwh;
    // Food habits emissions (flat rate per month)
    const foodTotal = exports.EMISSION_FACTORS.food[inputs.foodHabit || ''] || exports.EMISSION_FACTORS.food.mixed;
    // Shopping habits emissions (flat rate per month)
    const shoppingTotal = exports.EMISSION_FACTORS.shopping[inputs.shoppingHabit || ''] || exports.EMISSION_FACTORS.shopping.moderate;
